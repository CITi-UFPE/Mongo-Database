...
export default function AnimatedLogo({ className }: AnimatedLogoProps) {
  const gradientId = useMemo(
    () => `animated-logo-gradient-${Math.random().toString(36).slice(2)}`,
    []
  );
      ...
      <defs>
        <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
          <motion.stop
            offset="0%"
            stopColor="#0ea5e9"
            animate={{ stopColor: ["#0ea5e9", "#22d3ee", "#34d399", "#0ea5e9"] }}
            transition={{ duration: 7.5, repeat: Infinity, ease: "linear" }}
          />
          <motion.stop
            offset="100%"
            stopColor="#14b8a6"
            animate={{ stopColor: ["#14b8a6", "#0ea5e9", "#22d3ee", "#14b8a6"] }}
            transition={{ duration: 7.5, repeat: Infinity, ease: "linear", delay: 1.2 }}
          />
        </linearGradient>
      </defs>
